...
const INITIAL_ENEMY_SPEED = 1.125;
const INITIAL_SPAWN_COOLDOWN = 80;
...
const App: React.FC = () => {
  ...
  const update = useCallback(() => {
    ...
    if (framesSinceLastSpawnRef.current >= spawnCooldownRef.current && enemiesRef.current.length < maxEnemies) {
    ...
    <div className="flex flex-col h-screen bg-black text-white font-mono select-none overflow-hidden">
          ...
          <div className="flex items-center gap-6">
            <div className="flex gap-1 items-center bg-white/5 px-3 py-1 rounded-full border border-white/10">
              {Array.from({ length: 3 }).map((_, i) => (
                <svg key={i} className={`w-6 h-6 transition-all duration-300 ${i < lives ? 'text-red-500 fill-red-500 scale-100' : 'text-white/10 fill-white/10 scale-90'}`} viewBox="0 0 24 24">
            ...
            {gameState === GameState.PLAYING && (
              <button onClick={togglePause} className="p-2 bg-white/10 hover:bg-white/20 rounded-lg transition-colors border border-white/10">
        ...
        <main ref={containerRef} className="relative w-full max-w-[600px] h-full overflow-hidden">
          ...
          {gameState === GameState.PLAYING && (
            <div className="absolute top-4 left-4 p-3 bg-black/60 border border-white/10 rounded-xl flex items-center gap-4 shadow-xl backdrop-blur-md z-10">
              <div className="flex flex-col">
                ...
                <span className="text-sm font-bold mt-1" style={{ color: COLORS[currentAmmoIndex].hex }}>{COLORS[currentAmmoIndex].name}</span>
              </div>
              <div className="w-10 h-10 rounded-lg border-2 border-white/10 shadow-lg" style={{ backgroundColor: COLORS[currentAmmoIndex].hex, boxShadow: `0 0 20px ${COLORS[currentAmmoIndex].hex}44` }} />
              <div className="w-px h-8 bg-white/10" />
              <div className="flex flex-col">
                <span className="text-[10px] text-slate-500 uppercase font-black leading-none">Target</span>
                <span className="text-sm font-bold mt-1" style={{ color: COLORS[COLORS[currentAmmoIndex].compIndex].hex }}>{COLORS[COLORS[currentAmmoIndex].compIndex].name}</span>
              </div>
            </div>
          )}
          ...
          {gameState === GameState.START_SCREEN && (
            <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center p-6 text-center z-30">
              <h1 className="text-6xl font-black mb-4 text-white tracking-tighter italic drop-shadow-[0_0_20px_rgba(59,130,246,0.5)]">CHROMA<span className="text-blue-500">CANNON</span></h1>
              ...
              <button onClick={resetGame} className="px-10 py-4 bg-blue-600 hover:bg-blue-500 transition-all rounded-full font-black text-xl shadow-2xl border-b-4 border-blue-800">LAUNCH MISSION</button>
            </div>
          )}
          ...
          {gameState === GameState.PAUSED && (
            <div className="absolute inset-0 bg-blue-900/40 backdrop-blur-md flex flex-col items-center justify-center p-6 text-center z-30">
              ...
              <div className="bg-black/90 p-8 rounded-3xl border-2 border-blue-500/50 mb-8 shadow-2xl">
                <button onClick={() => setGameState(GameState.PLAYING)} className="px-12 py-4 bg-blue-600 text-white hover:bg-blue-500 transition-all rounded-xl font-black text-xl shadow-lg border-b-4 border-blue-800 mb-4 w-full">RESUME</button>
                <button onClick={() => setGameState(GameState.START_SCREEN)} className="px-12 py-3 bg-white/5 hover:bg-white/10 text-slate-400 hover:text-white transition-all rounded-xl font-bold text-sm border border-white/10 w-full">QUIT TO MENU</button>
              </div>
            </div>
          )}
          ...
          {gameState === GameState.CRASHED && (
            <div className="absolute inset-0 bg-yellow-900/40 backdrop-blur-md flex flex-col items-center justify-center p-6 text-center z-30">
              ...
              <div className="bg-black/90 p-8 rounded-3xl border-2 border-yellow-500/50 mb-8 shadow-2xl">
                <p className="text-slate-300 text-sm mb-4">Aliens are regrouping. Systems holding.</p>
                ...
                <button onClick={resumeFromCrash} className="mt-6 w-full px-12 py-5 bg-yellow-500 text-black hover:bg-yellow-400 transition-all rounded-full font-black text-2xl shadow-2xl active:scale-95 border-b-4 border-yellow-700">CONTINUE</button>
              </div>
            </div>
          )}
          ...
          {gameState === GameState.GAME_OVER && (
            <div className="absolute inset-0 bg-red-950/40 backdrop-blur-md flex flex-col items-center justify-center p-6 text-center z-30">
              ...
              <button onClick={resetGame} className="px-10 py-4 bg-white text-black hover:bg-gray-200 transition-all rounded-full font-black text-xl shadow-2xl active:scale-95">RE-TRY</button>
            </div>
          )}
        </main>
      </div>
      ...
      <footer className="h-44 bg-slate-950 border-t border-white/10 p-4 flex gap-8 items-center justify-center shadow-[0_-10px_30px_rgba(0,0,0,0.8)] z-20">
        <div className="flex gap-4">
          <button onPointerDown={() => setInputState('left', true)} onPointerUp={() => setInputState('left', false)} onPointerLeave={() => setInputState('left', false)} disabled={gameState !== GameState.PLAYING} className={`w-20 h-20 bg-white/5 rounded-2xl flex items-center justify-center border border-white/10 shadow-xl transition-all ${gameState !== GameState.PLAYING ? 'opacity-30' : 'active:bg-blue-600 active:scale-90'}`}>
          ...
          <button onPointerDown={() => setInputState('right', true)} onPointerUp={() => setInputState('right', false)} onPointerLeave={() => setInputState('right', false)} disabled={gameState !== GameState.PLAYING} className={`w-20 h-20 bg-white/5 rounded-2xl flex items-center justify-center border border-white/10 shadow-xl transition-all ${gameState !== GameState.PLAYING ? 'opacity-30' : 'active:bg-blue-600 active:scale-90'}`}>
        ...
        <button onPointerDown={(e) => { e.preventDefault(); if (gameState === GameState.CRASHED) resumeFromCrash(); else if (gameState === GameState.PAUSED) setGameState(GameState.PLAYING); else handleFire(); }} className={`w-32 h-32 rounded-full flex flex-col items-center justify-center shadow-2xl active:scale-90 transition-all border-8 ${gameState === GameState.PAUSED || gameState === GameState.CRASHED ? 'bg-yellow-600 border-yellow-700' : 'bg-red-600 border-red-700'}`}>
          <span className="font-black text-xl italic tracking-tighter text-white drop-shadow-md">{gameState === GameState.CRASHED || gameState === GameState.PAUSED ? 'RESUME' : 'FIRE'}</span>
        </button>
        <button onPointerDown={(e) => { e.preventDefault(); cycleAmmo('next'); }} disabled={gameState !== GameState.PLAYING} className={`w-40 h-20 bg-white/5 rounded-2xl flex items-center justify-center gap-3 shadow-lg transition-all border border-white/10 px-4 ${gameState !== GameState.PLAYING ? 'opacity-30' : 'active:bg-blue-600 active:scale-90'}`}>
          <span className="font-black text-xs uppercase text-slate-400">Swap<br/>(A/S)</span>
